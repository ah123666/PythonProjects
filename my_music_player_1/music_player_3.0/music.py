...
class Text:
    ...
    def __init__(self, src_rect, content, k):
        self.content = content
        self.line_gap = 1
        self.line_width = 2
        self.update(src_rect)
...
class Button:
    ...
    def update(self, rect, image):
        ...
        self.range_x, self.range_y = self.get_range()
...
class Music:
    ...
    def get_played_percent(self):
        current_time = time.time()
        played_time = current_time - self.start_time
        self.played_percent = played_time / self.length

    def draw_play_bar(self, src, scr_rect, button_rect1, button_rect2):
        self.get_played_percent()
        x_length = button_rect2.right - button_rect1.left
        y_length = int(scr_rect.height * 0.01)
        for y_pos in range(button_rect1.bottom + button_rect1.height, button_rect1.bottom + button_rect1.height + y_length):
            pygame.draw.line(src, color_red, (button_rect1.left, y_pos), (int(x_length * self.played_percent), y_pos), 1)
            pygame.draw.line(src, color_white, (int(x_length * self.played_percent) + 1, y_pos), (button_rect2.right, y_pos), 1)
...
while True:
    ...
    i += 1
    if pygame.mixer.music.get_busy() and i == 100:
        music.draw_play_bar(screen, screen_rect, last_button.rect, next_button.rect)
        i = 0
